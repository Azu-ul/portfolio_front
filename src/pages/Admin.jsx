...
const Admin = () => {
  ...
  // Estados para las confirmaciones de eliminación
  ...
  // Función para mostrar confirmación de eliminar proyecto
  const showDeleteProjectConfirm = (project) => {
    setConfirmDelete({
      show: true,
      type: 'project',
      id: project.id,
  ...
  // Función para eliminar proyecto confirmado
  const confirmDeleteProject = async () => {
    try {
      await axios.delete(`${API_BASE_URL}/api/admin/projects/${confirmDelete.id}`, { headers: apiHeaders });
      loadAllContent();
  ...
  // Función para mostrar confirmación de eliminar skill
  const showDeleteSkillConfirm = (skill) => {
    setConfirmDelete({
      show: true,
      type: 'skill',
      id: skill.id,
  ...
  // Función para eliminar skill confirmado
  const confirmDeleteSkill = async () => {
    try {
      await axios.delete(`${API_BASE_URL}/api/admin/skills/${confirmDelete.id}`, { headers: apiHeaders });
      loadAllContent();
  ...
  return (
    <>
              ...
              <div className="col-lg-6">
                ...
                {projects.map(project => (
                  <div key={project.id} style={{ paddingBottom: '20px', borderBottom: '1px solid #eee', marginBottom: '20px' }}>
                    ...
                    <Button variant="secondary" onClick={() => showDeleteProjectConfirm(project)}>Eliminar</Button>
                  </div>
                ))}
              </div>
            </div>
                      ...
                      {categorySkills.map(skill => (
                        <div key={skill.id} style={{ 
                          ...
                          <div>
                            ...
                            <Button variant="secondary" onClick={() => showDeleteSkillConfirm(skill)}>Eliminar</Button>
                          </div>
                        </div>
                      ))}
                    </div>
      ...
      <Overlay
        ...
        message={`¿Estás seguro de que quieres eliminar ${confirmDelete.type === 'project' ? 'el proyecto' : 'la habilidad'} "${confirmDelete.name}"? Esta acción no se puede deshacer.`}
